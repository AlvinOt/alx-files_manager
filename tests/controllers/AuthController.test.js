...
OAOAOAOAOAOAOAOA          expect(res.body.token.length).to.be.greaterThan(0);
OBOBOBOB          token = res.body.token;
OAOAOAOA          done();
        });
OAOAOAOA    });
  });
...
OAOAOAOA    it('+ Fails for a non-existent user', function (done) {
      this.timeout(5000);
OBOBOBOB      request.get('/disconnect')
        .set('X-Token', 'raboof')
        .expect(401)
        .end((requestErr, res) => {
          if (requestErr) {
