...
OAOAOAOAOAOAOAOAOAOAOAOAOAOAOAOA          .then(() => {
OAOAOAOA            if (callback) {
              callback();
            }
OAOAOAOA          })
...
OAOAOAOA        expect(res.body.email).to.eql(user.email);
OAOAOAOA        expect(res.body.id.length).to.be.greaterThan(0);
OBOBOBOB        if (callback) {
          callback();
        }
      });
  };
  ...
  after(function (done) {
OAOAOAOA    this.timeout(10000);
OAOAOAOA    setTimeout(() => {
OAOAOAOA      emptyDatabaseCollections(done);
      emptyFolder(baseDir);
    });
  });
OBOBOBOB
...
OAOAOAOA          }
          expect(res.body.id).to.exist;
          expect(res.body.userId).to.exist;
OAOAOAOA          expect(res.body.name).to.eql(mockFiles[1].name);
  ...
  describe('+ GET: /files', () => {
    ...
    it('+ Fetches first page with no page query', function (done) {
      ...
      request.get('/files')
        ...
        .expect(200)
        .end((requestErr, res) => {
OAOAOAOA          if (requestErr) {
OAOAOAOA            return done(requestErr);
OAOAOAOA          }
...
OAOAOAOAOAOAOAOA    it('+ Succeeds if file is linked to user', function (done) {
OAOAOAOAOAOAOAOA      this.timeout(5000);
